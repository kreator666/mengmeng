...
双击运行项目根目录的 **`启动系统.bat`** 即可：

- 首次运行会自动构建前端（需要本机已安装 Node.js）
- FastAPI 同时托管前端页面和后端 API，**单端口 8855**
- 访问地址：http://localhost:8855 （API 文档：http://localhost:8855/docs）
- 关闭命令行窗口即停止系统
- 代码更新后：双击 **`更新并重启.bat`** 重新构建前端，再重新启动

数据（K 线缓存、回测结果、自定义因子）保存在 `backend/data/`，重启不丢失。
